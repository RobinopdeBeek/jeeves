...
| `AcpBridge` | `server/ws/chat.ts` (+ `chat-session.ts`, `open-chat.ts`, `rewind-chat.ts`, `session-registry.ts`) | push-only `openSession` / `sendMessage` + `attach`/`onChunk`; headless `runToCompletion`; warm registry acquire/reattach on opaque ids; `ChatSession` + `openChat` / `rewindProjectChat` for adapters | spawning `agent acp`, ACP→`UIMessage` projection, permission responses (incl. headless cursor-like policy), JSON-RPC piping, warm cap/eviction, seed-once / nullable opener, disconnect/hand-off close, edit/branch rewind (truncate→close→respawn→reseed) |
