...
**Structured skill outputs** (e.g. `/to-tasks` emitting `{ title, description, depends_on }[]`) use **Zod-validated exchange files**: the skill writes JSON to a known worktree path, the runner harvests it, validates with a Zod schema on the host, and retries with error feedback on parse failure. We do **not** use AI SDK `generateObject`/`generateText` for these — that would introduce a second billing path (provider API keys) and bypass Cursor's codebase context, which `/to-tasks` specifically needs.
...
- `/to-tasks` and similar structured-output skills document their exchange file path and Zod schema alongside the prompt; parse failures surface as run errors with retry, not silent bad data.
