...
All AI inference runs on the Cursor subscription — via the ACP bridge for interactive chat and via Sandcastle + `cursor("composer-2.5")` for autonomous execution. We adopt **Vercel AI SDK 5** (`ai`, `@ai-sdk/react`) as the chat primitive: `useChat` owns message state, streaming, and typed `UIMessage` parts. **assistant-ui** (`@assistant-ui/react`, `@assistant-ui/react-ai-sdk`) sits on top for pre-built chat UI primitives (message list, composer, streaming indicators) rather than hand-rolling them. assistant-ui is a headless component layer over AI SDK, not a replacement for it.
...
- **LangChain / Mastra / CrewAI** — backend orchestration for building agent reasoning loops ourselves; Cursor's ACP session and Sandcastle runs already are the agent.
...
- `ExecutionEngine` tests mock `AgentRunner`; Sandcastle integration tests live behind the implementation.
