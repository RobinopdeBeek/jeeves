...
Keeping the steps separate preserves per-step progress, retry granularity, the human rework loop (`Implement changes →` re-queues `implement` and resets `ai-review` without re-planning), and per-step artifacts. Product-wise the three steps are `/implement` split across separate context windows (light plan → TDD implement → code-review + immediate rework); operationally they stay separate runs so the board can monitor and retry each stage.
