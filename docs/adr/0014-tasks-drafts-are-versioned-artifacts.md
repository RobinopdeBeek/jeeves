...
- Skill / exchange JSON may use `depends_on` indices; the host normalizes to stable `id` / `dependsOn` before appending a tip. On revise harvest, exchange tasks should carry tip `id`s so identity survives insert/reorder; unknown ids are rejected. Array index alone does not preserve ids.
...
- Undo is a durable tip-copy append (refresh-safe). Redo is not part of the tip history model.
