...
- Unlike Spec markdown (ADR 0005 in-round upsert), `tasks-draft` is **append-only within the round**; `latest` by `created_at` is the tip. Fan-out (later slice) freezes the tip as `tasks-breakdown` and prunes older draft versions.
