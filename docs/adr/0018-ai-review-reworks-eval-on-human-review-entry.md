...
The **Human Review Report** HTML is created by an explicit Review-column step, **Prepare Human Review** (`prepare-human-review`), which runs before **Human Review** and pins the report to the commit tip after AI Review. AI Review no longer feeds report fragments; earlier plan text that treated AI Review as the report pipeline (or forbade AI Review commits) is superseded. Slice 8 ships Plan → Implement → AI Review, advances into Review with `prepare-human-review` queued, and stubs Prepare Human Review enough to finish; slice 9 replaces that stub with the real assemble pipeline.
