...
The evaluation is a self-contained HTML report generated by the **Prepare Eval** step
(`prepare-eval`) when a card is in the Review column — after Plan → Implement → AI Review have
finished on the card tip. It is the artifact the **human** Review step works from. The AI
Review step produces only a concise markdown overview and may rework code; it does **not**
assemble the Evaluation ([ADR 0018](../adr/0018-ai-review-reworks-eval-on-human-review-entry.md)).
...
**Task Evaluation — the deep review (Prepare Eval + Human Review, on each child branch).**
This is the primary human review surface. Because the tasks are vertical slices, each one is
independently end-to-end testable, so each gets the full eval pipeline below: a small focused
diff narrative, the slice's tests, and a targeted QA checklist. Autonomous dual-axis code
review + rework already happened in the **AI Review** step before Prepare Eval. Small diffs
keep the review fast and notifications attributable to a single unmerged slice you can still
reject.
...
| (AI Review markdown overview is separate, pre-eval) | Spec acceptance-criteria checklist |
...
Each section is its own skill, run sequentially under **Prepare Eval** after AI Review:

```
/ai-review  (dual-axis review + rework; review.md)   ← Implement Task
      ↓
Prepare Eval (`prepare-eval`):
...
/eval-assemble         → Combines all fragments + consolidates Notifications
                         → writes self-contained HTML file + `notifications.json` exchange file
                         → runner harvests both into `<repo>/.jeeves/data/` / DB
                         → human Review step becomes needs-user
```
...
refactor-opportunities pass — but **no** diff narrative (those live on each child's Task
Evaluation, linked from the Tasks section).
...
The **Prepare Eval** step shows a mini-pipeline in the RunLog (“Preparing interactive
evaluation…”), not just a single "running" state:
...
Human Review → Prepare Eval
  ...
  ✓ /eval-tests             (15s)
  ⟳ /eval-qa-plan           ← running
  ○ /eval-assemble
```

When complete, the human Review step is `needs-user` and the assembled evaluation is the
