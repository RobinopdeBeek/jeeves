...
Stage 11: /eval-acceptance             [ai-execution]  ← feature-level integration + PRD acceptance eval
Stage 12: Human acceptance review      [human]         ← final gate before PR to main
Stage 13: /deploy                      [ai-execution]  ← opens PR: feat/my-feature → main
```

The evaluation happens at **two levels** (see [The Evaluation Plan](#the-evaluation-plan)):
the per-task eval (stage 8, on each child branch) is the deep, surface-everything review of
one slice; `/eval-acceptance` (stage 11, on the feature branch) is a scoped integration +
acceptance gate over the whole assembled feature. They have deliberately different scopes so
the feature-level pass is not a second full review.
...
feat/my-feature
  ...
  → /eval-acceptance        ← feature-level eval runs HERE, on the feature branch
  → human acceptance review
  → /deploy
  → PR opened: feat/my-feature → main
```

**Where each eval runs:** per-task evals run on the child branch (`feat/my-feature/card-N`)
*before* it merges, so each slice is reviewed against the cumulative state of its predecessors
(slices merge sequentially, so card-N's branch already contains card-1…N-1). The feature-level
acceptance eval runs on `feat/my-feature` after all slices are merged and architecture is
improved, just before the PR to `main`.
...
**Per-task eval — the deep review (stage 8, on each child branch).**
...
unmerged slice you can still reject at stage 9. This is where the surface-everything work lives.

**Feature-level eval — the acceptance gate (stage 11, `/eval-acceptance`, on the feature branch).**
After all slices are merged and `/improve-architecture` has run, a *thinner* eval validates the
assembled feature as a whole. It deliberately omits the things the per-task evals already
covered and focuses on what only emerges once everything is integrated:

| Per-task eval (stage 8, child branch) | Feature-level eval (stage 11, feature branch) |
|---|---|
...
at before approving. For example: Deviation from plan / Test gap / Critical Review finding /
...
└── .sandcastle/
    ...
    │   ├── eval-acceptance.md        # feature-level integration + PRD acceptance eval
    │   └── deploy.md
...
- `/eval-acceptance` (stage 11) — feature-level eval on `feat/my-feature` after architecture
  improvement. Reuses the stage-8 eval skills but feature-scoped: full regression run,
  cross-slice end-to-end journeys, PRD acceptance checklist, holistic screenshot walkthrough.
  Skips the diff narrative — links back to each child card's per-task eval plan instead. Emits
  attention flags consolidated for the human acceptance review (stage 12) before `/deploy`.
